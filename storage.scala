...
object storage {
  ...
  val constant : Int => () => Int = {
    (n:Int) => (() => {   // anon function that returns a function that returns the initial n.
      val initalN = n
      initalN
    });
  }
  ...
  val counter : Int => () => Int = {
    ...
    (someN: Int) => {
      var initialN = someN  // available through the whole outer anon function.
      () => {
        var compute = initialN  // we store the initial value since we want this inner function to return it.
        initialN = initialN + 1 // change the value.
        compute // return the initial value.
      }
    }
  }
  ...
  val getAndSet : Int => (() => Int, Int => Unit) = {
    (n:Int) => {
      var someN = n;
      (  ()=> { someN }, (aN: Int) => {someN = aN} )    // A tupple with anon functions.
    }
  }
  ...
  val getAndSetSpy : () => (() => Int, Int => (() => Int, Int => Unit)) = {
    () => {
      var counter = 0 // exists throughout the whole outermost anon function.
      (   () => {counter}, (n: Int) => {var someN = n; (() => someN, (aN: Int) => {counter = counter + 1; someN = aN})}       )
    }
  }
}

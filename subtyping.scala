...
object subtyping {
  ...
  /* My class that extends Counter */
  class Tracker extends Counter {
    private var n = 0
    var track = 0 // A var that keeps tracks of how many times increment and decrement gets called.
    override def increment() = { n = n + 1; track = track + 1 }
    override def decrement() = { n = n - 1; track = track + 1 }
    override def get(): Int = { n }
    def getTrack(): Int = { track }
  }

  def observeCounter (f : Counter => Unit) : Int = {
    val result = new Tracker ()
    f (result)
    var total = result.getTrack()
    total
  }
  ...
  def observeCounterList (f : List[Counter] => Unit) : List[Int] = {
     val myList: List[Tracker] = List[Tracker](new Tracker(), new Tracker(), new Tracker())
     if(myList.length < 3) { Nil }
     f (myList) // on each element of type List[Tracker]
     val listResult = List(myList(0).getTrack(), myList(1).getTrack(), myList(2).getTrack())
     listResult
  }
  ...
  def observeCounterArray (f : Array[Counter] => Unit) : Array[Int] = {
    val myArray: List[Tracker] = List(new Tracker(), new Tracker(), new Tracker())
    f (myArray.toArray)
    List(myArray(0).getTrack(), myArray(1).getTrack(), myArray(2).getTrack()).toArray

    // TODO: The thing below does not work and I don't know why...
    // val myArray: Array[Tracker] = Array(new Tracker(), new Tracker(), new Tracker())
    // f (myArray)
    // List(myArray().getTrack(), myArray(1).getTrack(), myArray(2).getTrack()).toArray
  }
}

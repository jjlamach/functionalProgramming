...
object argpass {
  ...
  def refint1 (f : RefInt => Unit) : (Int, Int, Int) = {
    //TODO: Provid definition here.
    val myRef = new RefInt(0)
    f(myRef)
    val res1 = myRef.get
    f(myRef)
    val res2 = myRef.get
  ...
  def refint2 (f : RefInt => Unit) : (Int, Int, Int) = {
    ...
    val myRef = new RefInt(0)
    val myRef2 = new RefInt(0)
    val myRef3 = new RefInt(0)
    f(myRef)
    f(myRef2)
    f(myRef3)
    (myRef.get, myRef2.get, myRef3.get)
  }
  ...
  def refint3 (r : RefInt) : RefInt = {
    // TODO: Provide definition here.
    var result = new RefInt(0);
    var doubleTheOriginal = r.get * 2
    result.set(doubleTheOriginal)

    var tmp = r // since they are val, we copy the reference we get in a variable to work with it
    var computation = tmp.get + 1 // Your code must increment (add 1 to) the RefInt it receives
    tmp.set(computation)
    result
  }
  ...
  def refint5 (xs : List[RefInt]) : Unit = {
    // TODO: Provide definition here.
    var myList: List[RefInt] = xs
    for (x <- myList) {   // Set to 2
      x.set(2)
    }
  }
}

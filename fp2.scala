...
object fp2 {
  ...
  def filter [A] (xs:List[A], f:A=>Boolean) : List[A] = {
    ...
    xs match {
      ...
      case _::ys => filter(ys, f) // default case
    }
  }
  ...
  def joinTerminateLeft (xs : List[String], term : String) : String = {
    /*
      will pass this function to foldLeft so that it can be applied to each element
      that is in 'xs'
    */
    def helper(x:String, y:String): String = {
      x + y + term
    }
    xs match {
      case Nil => ""
        // starting point: ys, then function, then return type of helper
      case y::ys => foldLeft(ys, helper("", y), helper)
    }
  }
  ...
  def joinTerminateRight (xs : List[String], delimiter : String) : String = {
    ...
    xs match {
      case Nil => ""
      case y::ys => y + foldRight(ys, delimiter, helper)
    }
  }
  ...
  def firstIndexHelper(a:Int, xs:List[Int], i:Int): Int = {
    xs match {
      case Nil => throw new RuntimeException()
      case u::us => if(u >= a) i else firstIndexHelper(a, us, i + 1)
    }
  }
